...
# --- Automatically find the 'eta' variable index from the Exodus file ---
...
# Read the 'eta' variable data
param = model.variables[f'vals_nod_var{variable_index}'][:]

# Interpolate to structured grid and transpose to (time, y, x)
# Note: No scaling is applied here because 'eta' is a dimensionless order parameter (typically 0 to 1)
ETA = np.transpose(make_array_parallel(nodes_block, X_all, Y_all, param, blockname="domain", n_jobs=6, ny=50, nx=75), 
                   axes=(2, 0, 1))

print(f"ETA array shape: {ETA.shape}")

# Save to .npy file
numpy_path = os.path.abspath(f'./results/{power_level}/')
np.save(numpy_path+'/ETA.npy', ETA)
np.save(numpy_path+'/time.npy', time_real)

print("\n Job Done: Saved ETA.npy")
...
if t_step < ETA.shape[0]:
    hmap1 = ax1.imshow(ETA[t_step], cmap=cmap, extent=[0, 75, 0, 50], aspect=1.0, interpolation='quadric')
    ax1.set_title(f"Order Parameter (eta) at t = {time_real[t_step]:.2f}")
    plt.colorbar(hmap1, ax=ax1, label='eta')
else:
    print(f"Timestep {t_step} out of bounds (max {ETA.shape[0]-1})")
