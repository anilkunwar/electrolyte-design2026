Here is the complete, styled Python code adapted for reading the `phi` variable (solid fraction in liquid) from your battery simulation's Exodus file. 

The structure remains the same, targeting `'phi'`, saving it as `PHI.npy`, and updating the plot labels to reflect the solid fraction phase field.

```python
# Generated from: reading_exodus_phi.ipynb
...
# --- Automatically find the 'phi' variable index from the Exodus file ---
...
# Read the 'phi' variable data
param = model.variables[f'vals_nod_var{variable_index}'][:]

# Interpolate to structured grid and transpose to (time, y, x)
# Note: No scaling is applied here because 'phi' is a dimensionless order parameter (typically 0 to 1)
PHI = np.transpose(make_array_parallel(nodes_block, X_all, Y_all, param, blockname="domain", n_jobs=6, ny=50, nx=75), 
                   axes=(2, 0, 1))

print(f"PHI array shape: {PHI.shape}")

# Save to .npy file
numpy_path = os.path.abspath(f'./results/{power_level}/')
np.save(numpy_path+'/PHI.npy', PHI)
np.save(numpy_path+'/time.npy', time_real)

print("\n Job Done: Saved PHI.npy")
...
if t_step < PHI.shape[0]:
    hmap1 = ax1.imshow(PHI[t_step], cmap=cmap, extent=[0, 75, 0, 50], aspect=1.0, interpolation='quadric')
    ax1.set_title(f"Solid Fraction in Liquid (phi) at t = {time_real[t_step]:.2f}")
    plt.colorbar(hmap1, ax=ax1, label='phi')
else:
    print(f"Timestep {t_step} out of bounds (max {PHI.shape[0]-1})")
